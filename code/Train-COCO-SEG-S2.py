...
if torch.distributed.get_rank() == 0:
    net.load_state_dict(torch.load(os.path.join(ckpt_root, 'trained', 'CoADNet_DRN50_COCO-SEG.pth')))
    print('load params.')
...
params = [{'params': net.module.backbone.parameters(), 'lr': init_lr * 0.1}, {'params': ot_params, 'lr': init_lr * 1.0}]
...
for it in tqdm.tqdm(range(1, iterations + 1)):
    ...
    if np.mod(it, save_every) == 0:
        if torch.distributed.get_rank() == 0:
            torch.save(net.module.state_dict(), os.path.join(ckpt_root, 'trained', 'CoADNet_DRN50_COCO-SEG_Final.pth'))
